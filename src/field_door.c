...
{
    const struct DoorAnimFrame *doorAnimFrames = sDoorAnimFrames_OpenSmallFrlg;
...
static s8 StartDoorOpenAnimation(const struct DoorGraphics *gfx, u32 x, u32 y)
{
    ...
    {
        if (gfx->size == 2)
            return StartDoorAnimationTask(gfx,sDoorAnimFrames_OpenLargeFrlg, x, y);
        else
            return StartDoorAnimationTask(gfx,sDoorAnimFrames_OpenSmallFrlg, x, y);
    }
}
...
static s8 StartDoorCloseAnimation(const struct DoorGraphics *gfx, u32 x, u32 y)
{
    ...
    if (gfx == NULL)
        ...
        return StartDoorAnimationTask(gfx,sDoorAnimFrames_CloseSmallFrlg, x, y);
}
